...
def extract_first_table_llm(pdf_path: Path, strategy="lines_strict", page=None):
    """
    ...
    print(f"\nSearching for first table with strategy '{strategy}'...")
    print(f"Total chunks (pages): {len(chunks)}")
    
    # Search for the first table in any page
    for chunk_idx, chunk in enumerate(chunks):
        tables = chunk.get("tables") or []
        print(f"  Chunk {chunk_idx + 1}: {len(tables)} table(s) found")
        
        if tables:
            table = tables[0]
            print(f"  First table found in chunk {chunk_idx + 1}")
...
def extract_first_table_pymupdf(pdf_path: Path, strategy="lines_strict", page=None):
    """
    ...
    print(f"\nTrying PyMuPDF directly with strategy '{strategy}'...")
