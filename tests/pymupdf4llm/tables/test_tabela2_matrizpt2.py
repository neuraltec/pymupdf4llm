...
def extrair_primeira_tabela_llm(pdf_path: Path, strategy="lines_strict", pagina=10):
    """
...
def extrair_primeira_tabela_pymupdf(pdf_path: Path, strategy="lines_strict", pagina=10):
    """
...
def test_primeira_tabela_com_llm(pdf_teste):
    ...
    for strategy in estrategias:
        tabela, estrutura_completa = extrair_primeira_tabela_llm(pdf_teste, strategy, pagina=10)
    ...
    if tabela is None:
        print("\npymupdf4llm não encontrou tabelas na página 10, tentando PyMuPDF diretamente...")
        for strategy in estrategias:
            tabela, estrutura_completa = extrair_primeira_tabela_pymupdf(pdf_teste, strategy, pagina=10)
            if tabela is not None:
                estrategia_usada = strategy
    ...
    esperado = {
        (0, 0): "Sl no.",
        (0, 1): "Name of the impurity",
        (0, 3): "Structure",
        (0, 4): "Origin",
        (1, 0): "Control",
        (1, 1): "1",
        (1, 2): "7-HDQ [7-Hydroxy-3,4-dihydroquinolin-2(1H)-one]",
        (1, 3): "Key starting material",
        (1, 4): "Existing HPLC method, employed to detect related substances in the drug substance is capable of resolving 7-HDQ, if present.",
        (2, 0): "2",
        (2, 1): "DCPP [1-(2,3-Dichlorophenyl)piperazine hydrochloride]",
        (2, 2): "",
        (2, 3): "Key starting material",
        (2, 4): "Existing HPLC method, employed to detect related substances in the drug substance is capable of resolving DCPP, if present.",
    }
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
| 1      | 7-HDQ                                                        |           | Key starting material| Existing HPLC method, employed to detect related substances in the drug | 
|        | [7-Hydroxy-3,4-dihydroquinolin-2(1H)-one]                    |           |                      | substance is capable of resolving 7-HDQ, if present.                    |
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
| 2      | DCPP                                                         |           | Key starting material| Existing HPLC method, employed to detect related substances in the drug |   
|        | [1-(2,3-Dichlorophenyl)piperazine hydrochloride]             |           |                      | substance is capable of resolving DCPP, if present.                     |
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    """
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------"""
    ...
    for strategy in estrategias:
        ...
        # Busca especificamente na página 10 
        idx_pagina = 10 - 1
        if idx_pagina < len(chunks):
            ch = chunks[idx_pagina]
            tabelas = ch.get("tables") or []
